...
def get_face(img_path,real_path):
    classifier = cv2.CascadeClassifier(r'..\DataSets\model\lbpcascade_animeface.xml')
...
def savepicture(soup,url_list,num):
...
def GetBigPicture(soup):
    ...
    for img_url in img_urls:
        try :
            ...
            img_url_list.append("https://safebooru.org/"+url)
            print(img_url_list)
...
def colect_girl_images(limit,init=0,num=0):
    ...
    while 1:
        ...
        GetBigPicture(soup)
        imgs=savepicture(soup,url_list,num)
